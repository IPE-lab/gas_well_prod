...
## casing pressure, production and other variables for 10 consecutive days as input features x_train. 
## The data from the 11th day is used as the label.
for i in range(10, len(training_set_scaled) - 10):
    x_train.append(training_set_scaled[i - 10:i, 0:6])
...
x_train = np.reshape(x_train, (x_train.shape[0], 10, 6))
...
for i in range(10, (len(test_set_scaled) - 10), 1):
    x_test.append(test_set_scaled[i - 10:i, 0:6])
    y_test.append(test_set_scaled[i, 2])
x_test, y_test = np.array(x_test), np.array(y_test)
x_test = np.reshape(x_test, (x_test.shape[0], 10, 6))
...
choose_neural_network = input("Please select GRU (input GRU) or LSTM (input LSTM) or RNN (input RNN) for training the model: ")
if choose_neural_network == "GRU":
    ## GRU, Dropout: 0.2
    model = tf.keras.Sequential()
    model.add(GRU(90, input_shape=(10, 6), activation="tanh", return_sequences=True))
    model.add(Dropout(0.2))
    model.add(GRU(90, input_shape=(10, 6), activation="tanh"))
    model.add(Dropout(0.2))
    model.add(Dense(1, activation="tanh"))
    model.compile(optimizer=tf.keras.optimizers.Adam(0.00004),
                loss='mse')
    
if choose_neural_network == "LSTM":
    ## LSTM, Dropout: 0.5
    model = tf.keras.Sequential()
    model.add(LSTM(90, input_shape=(10, 6),activation="tanh",return_sequences=True))
    model.add(Dropout(0.5))
    model.add(LSTM(90, input_shape=(10, 6),activation="tanh"))
    model.add(Dropout(0.5))
    model.add(Dense(1,activation="tanh"))
    model.compile(optimizer=tf.keras.optimizers.Adam(0.00004),
              loss='mse')
    
if choose_neural_network == "RNN":
    ## RNN, Dropout: 0.2
    model=tf.keras.Sequential()
    model.add(SimpleRNN(90, input_shape=(10,6),activation="tanh",return_sequences=True))
    model.add(Dropout(0.2))
    model.add(SimpleRNN(90, input_shape=(10,6),activation="tanh"))
    model.add(Dropout(0.2))
    model.add(Dense(1,activation="tanh"))
    model.compile(optimizer=tf.keras.optimizers.Adam(0.00004),
                loss='mse')
...
history = model.fit(x_train, y_train, batch_size=32, epochs=170, validation_data=(x_test, y_test), verbose=2,
                    shuffle=False)
model.summary()
model.save(f"./my_model/{choose_neural_network}_model_10_1.h5")
...
#     ## Create model
#     model=tf.keras.Sequential()
#     model.add(LSTM(80, input_shape=(10,7),activation="tanh"))
